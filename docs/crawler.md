...
- 正常抓取完成后会输出列表请求和详情请求的成功/失败统计。列表失败会记录排序方式、页码、归类后的失败原因和错误信息，并在统计开头按原因汇总（频控限速、连接异常、请求超时、SSL 证书校验失败、响应非 JSON 等）；详情失败会记录商品 ID。详情失败不超过 20 条时终端展开 ID，超过 20 条时终端只显示数量，完整 ID 仍写入本次日志。
- 触发 429 频控时按指数退避重试，单次等待封顶 60 秒；连续 3 次频控会刷新设备指纹；连接级故障（断连、连接池污染）会重建 Session 后重试。`requests` 未安装时回退到 `urllib`，退避策略一致。
- 每次抓取结束会把本次商品数与最近 5 次同分类、同 IP、同请求页数的成功批次对比，低于其均值 50% 时在终端和日志中告警。反爬生效通常表现为静默少给数据而不是报错，该告警用于提前发现截断。基线均值低于 50 条时不判断，避免小样本误报。
