...
The search filter row on the product page is a stack of two bands rather than one line. The 64px `{component.search-bar-pill}` keeps a band of its own, and the 48px dense controls (分类 / IP 分类 / 排序) sit in the row beneath it, each with its label above. The two heights are never mixed on a single row: a 64px pill beside a 48px control aligns on neither edge, and their labels — the pill's `{typography.caption}` over the segment and the filter label above the dense control — end up 8px apart on two different baselines. Within the dense row the select triggers are fixed-width so the row does not reflow when a long IP name is selected, and the sort segment group anchors the right edge, echoing the `space-between` rhythm of the section heading below it.
